...
import {type ofetch, createFetch} from 'ofetch';
import {glowUpOfetchWithLinks} from './glow-up-ofetch-with-links.js';

describe('glowUpOfetchWithLinks', () => {
	describe.runIf(isNode())('ofetch', () => {
		...
		test('links method filters links by rel', async ({expect}) => {
			const fetchWithLink = glowUpOfetchWithLinks({
	...
	test('glowUpOfetchWithLinks adds links method to response', async ({
		expect,
	}) => {
	...
	test('glowUpOfetchWithLinks handles multiple link headers', async ({
		expect,
	}) => {
		const fetchImpl = async (
			...arguments_: Parameters<(typeof ofetch)['raw']>
	...
	test('glowUpOfetchWithLinks handles empty link header', async ({expect}) => {
		const fetchImpl = async (
			...arguments_: Parameters<(typeof ofetch)['raw']>
		) =>
			new MockResponse(undefined, {
	...
	test('glowUpOfetchWithLinks passes through fetch errors', async ({
		expect,
	}) => {
		const fetchImpl = async (
			...arguments_: Parameters<(typeof ofetch)['raw']>
	...
	test('glowUpOfetchWithLinks passes through multiple fetch arguments', async ({
		expect,
	}) => {
	...
	test('glowUpOfetchWithLinks passes through single fetch argument', async ({
		expect,
	}) => {
		const mockFetch = vi.fn(
			async (...arguments_: Parameters<(typeof ofetch)['raw']>) =>
	...
	test('glowUpOfetchWithLinks follows RFC8288 links', async ({expect}) => {
		const fetchImpl = async (
			...arguments_: Parameters<(typeof ofetch)['raw']>
		) => {
	...
	describe('glowUpOfetchWithLinks fragments', () => {
		...
		test('returns a fragment from a JSON response', async ({expect}) => {
			...
			const fetchFragmentImpl = glowUpOfetchWithLinks(mockFetchImpl);
		...
		test('can fetch linked fragments', async ({expect}) => {
			...
			const decoratedFetch = glowUpOfetchWithLinks(mockFetchImpl);
